...
## [Correlation coefficient]{.page-concept}

- The sample Pearson [**correlation coefficient**]{.blue}, denoted as $r$, is a measure of the strength of a linear relationship between two variables ($x$ and $y$).
...
![](/images/xkcd/correlation.png)<br>
[Source: [xkcd](https://xkcd.com/)]{.f2}

- Correlation doesn't provide the direction of the relationship.
- We cannot say just because $x$ and $y$ are highly correlated, $x$ causes $y$ or vice versa -- [**correlation is not causation**]{.blue}!
...
Correlation coefficient: `r cor(data$x, data$y)`
...
## [Hypothesis testing for regression parameters]{.page-rproj}

![](/images/xkcd/slope_hypothesis_testing.png)
...
## [Confidence interval and prediction interval]{.page-rproj}

- The 95% confidence interval for the mean response at $x = 5$ is

```{webr}
predict(fit_sqrt, newdata = data.frame(Length = 5), interval = "confidence", level = 0.95)
```

- The 95% prediction interval for the response at $x = 5$ is

```{webr}
predict(fit_sqrt, newdata = data.frame(Length = 5), interval = "prediction", level = 0.95)
```

- The prediction interval is always wider as it considers the uncertainty in the response as well.


## [Functions for model object]{.page-rproj}
